...
class DpTableEntry():
    ...
    def get_left_child(self, dp_table : DpTable):
        ...
        lcx, lcy = CYKAlgo2.get_left_child_point(self.x, self.y, self.delta)
    ...
    def get_right_child(self, dp_table : DpTable):
        ...
        rcx, rcy = CYKAlgo2.get_right_child_point(self.x, self.y, self.delta)
...
class CYKAlgo2:
    ...
    def parse(self, tokens : list[Token]):
        self.n = len(tokens)
        self.tokens = CYKAlgo2.tokens_to_clrtoken(tokens)
    ...
    def _fill_diagonal(self, diagonal_number : int):
        ...
        for point in points:
            ...
            for delta in range(diagonal_number):
                lchild_x, lchild_y = CYKAlgo2.get_left_child_point(*point, delta)
                lrule_names = list(map(lambda x: x.name, self.dp_table[lchild_x][lchild_y]))

                rchild_x, rchild_y = CYKAlgo2.get_right_child_point(*point, delta)
